...
indent = 0.18; %Length of indentation (cm)
...
k = 1; %Initial Spring constant in quasiequilibrium state
...
while index_force < 500
    for index_n = 1:75 %y   m - 1 1:18
        for index_m = 1:19 %x
            ...
            %Hypothesis 2
            % 
            if nudgenode_x(index_n, index_m) <= cuffend && nudgenode_x(index_n, index_m) >= cuffstart
                if index_m == row_bot && (index_n ~= 1&& index_n ~= 75 )
                    kp_ne = 0;
                    kp_nw = 0;
                
                elseif index_m == row_top && (index_n ~= 1&& index_n ~= 75 )
                    kp_se = 0;
                    kp_sw = 0;
...
%The edge nodes will stay constant
            if index_m == row_bot && (index_n ~= 1&& index_n ~= 75 )
                if max(nudgenode_y(index_n, index_m + 1 ), nudge_temp_y(index_n, index_m)) == nudge_temp_y(index_n, index_m)
                    nudge_temp_y(index_n, index_m ) = nudgenode_y(index_n, index_m + 1);
                end
            elseif index_m == row_top && (index_n ~= 1&& index_n ~= 75 )
                if max(nudge_temp_y(index_n, index_m - 1 ), nudge_temp_y(index_n, index_m)) == nudgenode_y(index_n, index_m )
                    nudge_temp_y(index_n, index_m ) = nudgenode_y(index_n, index_m - 1);
                end
            end
...
%This codes for the artery display

yline(((mean(nudgenode_y(35, row_top)) + mean(nudgenode_y(35,row_bot)))/ 2) , "r-");
xlabel("Length(cm)");
ylabel("Height(cm)");
title({['Compression of Tissue with ' ,num2str(cuffwidth),'cm'], ['Blood Pressure Cuff with ',num2str((1 - compressionfactor) * 100),' % Depression']});
%k
hold off
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for index_n = 1:75
    dia_dif(index_n, 1) = nudgenode_y(index_n, row_top) - nudgenode_y(index_n, row_bot);
end

figure(4)
plot(nudgenode_x(:, 1),dia_dif, 'b') 
xlabel("Length(cm)")
ylabel("Internal Artery Diameter(cm)")
title({['Average Internal Artery Diamter(cm) across the ', num2str(model_width), ' cm'], ['with compression with ', num2str(cuffwidth) 'cm']}); 
