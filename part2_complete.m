...
pre_pla1ceholder_Cpp = zeros(1,1001);
...
for time = deltaT:deltaT:endTime
    ...
    %flowin = (Pmax * power(Ad, 2))  / (2 * Kd);
    %flowout = Plung / Rair;
    dPlung = ((133* 1000* dDPext_dt * power(Ad, 2))  / Kd - (Plung(index) / Rair)) * (1 / Clung) ; %(12)
    
    
    %dPlung = (1 / Clung) * (((power(Ad, 2) * dDPext_dt) / Kd) - (Plung / Rair));
    %dPlung = (1 / Clung) * (flowin - flowout);
    %flowair = Plung(index) / Rair;
    %dVlung = dPlung * deltaT; %not even used later
    %Vol = flowair * time;
    index = index + 1;
    %part 12
    %Lung Prssure
    %dPlung / dt = (Ad ^2) * 
	Plung(index) = Plung(index - 1) + (deltaT * dPlung); %%Pressure
    Vol(index) = (Plung(index) / Rair) * deltaT + Vol(index - 1); %Volume
    %part 13
